Ext.define('Buzzor.prj1.prjone.controller.Contacts.CommunicationGroupController',
{
	extend : 'Ext.app.ViewController',
	alias : 'controller.CommunicationGroupController',
	init:function()
	{

	var storeCommunicationType = new Ext.data.Store({ fields : [],data : []    });
gridCommunicationType = this.view.down('#CommunicationTypeGrid'); 
gridCommunicationType.setStore(storeCommunicationType)
	},
...
addCommunicationType : function(){
	var formCommunicationType = this.view.down('#CommunicationTypeForm');
	var gridCommunicationType = this.view.down('#CommunicationTypeGrid');
	 if(formCommunicationType.isValid()){
		gridCommunicationType.getStore().add(formCommunicationType.getValues());	}
	},


	
onCommunicationTypeGridItemClick : function(cellModel, record, rowIndex, columnIndex, eOpts){
	form = this.view.down('#CommunicationTypeForm');
...
if (data.id != null ) {delete data.id;}var addCommunicationType = this.view.down('#CommunicationTypeGrid');
var addCommunicationTypeBindLavel = addCommunicationType.bindLevel;
	delete data[addCommunicationTypeBindLavel];
	this.createObject(data, addCommunicationTypeBindLavel, this.fetchDataFromStore(addCommunicationType.getStore()));
var jsonData = {};
		for(key in data){
...
refreshMainForm : function(){debugger;mainView = this.view.up().up().up();mainView.controller.renderTreeData();this.resetForm();},resetForm : function() {this.view.getForm().reset();
	var gridCommunicationType = this.view.down('#CommunicationTypeGrid');
		gridCommunicationType.getStore().removeAll();},
