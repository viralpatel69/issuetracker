Ext.define('Buzzor.prj1.prjone.controller.Location.VillageController',
{
	extend : 'Ext.app.ViewController',
	alias : 'controller.VillageController',
	init:function()
	{
		this.countryIdLoad();
		this.talukaIdLoad();
	...
	onstateIdChange: function (primaryKey){
		...
		Ext.Ajax.request(
		{
		 ...
		 jsonData : {
			...
			sender:scope,
			success : function(response,scope){
				var districtIdCombo = scope.sender.down('#districtId');
				var storedata = Ext.JSON.decode(response.responseText);
				storedata = storedata.response.data;
				var myStore = Ext.create('Ext.data.Store', {
					fields : [],
				...
				districtIdCombo.setStore(myStore);
districtIdCombo.getStore().sort('primaryDisplay','ASC');			scope.sender.controller.addCommunicationLog(response, daoInfo, true);},failure : function(response, scope){debugger;var daoInfo = scope.sender.down('#countryId');scope.sender.controller.addCommunicationLog(response, daoInfo, false);}
		},scope);
	},
...
stateIdCombo.getStore().sort('primaryDisplay','ASC');			scope.sender.controller.addCommunicationLog(response, daoInfo, true);},failure : function(response, scope){debugger;var daoInfo = scope.sender.down('#countryId');scope.sender.controller.addCommunicationLog(response, daoInfo, false);}
	...
	talukaIdLoad: function (){
		var scope = this.getView();
		Ext.Ajax.request(
		{
			url: 'secure/Taluka/findAll',
			method:'GET', 
			sender:scope,
			jsonData :{},
			success : function(response,scope){
				var daoInfo = scope.sender.down('#talukaId');
				var storedata = Ext.JSON.decode(response.responseText);
				storedata = storedata.response.data;
				var myStore = Ext.create('Ext.data.Store', {
					fields : [],
				...
				daoInfo.setStore(myStore);
daoInfo.getStore().sort('primaryDisplay', 'ASC');queryPanel =  scope.sender.up().up().up().down('#queryPanel');if(queryPanel.down('#talukaId') != null){queryPanel.down('#talukaId').setStore(myStore);queryPanel.down('#talukaId').getStore().sort('primaryDisplay', 'ASC');}scope.sender.controller.addCommunicationLog(response, daoInfo, true);			},failure : function(response, scope){debugger;var daoInfo = scope.sender.down('#countryId');scope.sender.controller.addCommunicationLog(response, daoInfo, false);}
		},scope);
	},
